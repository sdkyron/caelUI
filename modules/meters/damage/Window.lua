--[[	$Id: Window.lua 3955 2014-10-28 10:08:52Z sdkyron@gmail.com $	]]
...
RegisterStateDriver(window, "visibility", "[petbattle] hide; show")
...
local menuTable = {
	...
	{text = RESET, func = function() addon:Reset() end, notCheckable = true},
}
...
function window:OnInitialize()
	...
	self:SetSize(pixelScale(s.width), pixelScale(s.titleheight + s.maxlines * (s.lineheight + s.linegap)))
	...
	scroll:SetTexCoord(0.8, 1, 0.8, 1)
	...
	reset:SetPoint("TOPRIGHT", pixelScale(-1), pixelScale(-1))
	...
	reset.text:SetPoint("CENTER", pixelScale(1), 0)
	reset.text:SetText(">")
	...
	segment:SetSize(pixelScale(s.titleheight - 2), pixelScale(s.titleheight - 2))
	segment:SetPoint("RIGHT", reset, "LEFT", pixelScale(-2), 0)
	...
	title:SetVertexColor(0.25, 0.66, 0.35, s.titlealpha)
	title:SetPoint("TOPLEFT", pixelScale(1), pixelScale(-1))
	title:SetPoint("BOTTOMRIGHT", reset, "BOTTOMLEFT", pixelScale(-1), 0)
	...
	font:SetPoint("LEFT", title, "LEFT", pixelScale(4), 0)
	font:SetPoint("RIGHT", segment, "LEFT", pixelScale(-1), 0)
...
function window:SetScrollPosition(curPos, maxPos)
	...
	self.scroll:SetHeight(pixelScale(s.maxlines / maxPos * total))
	self.scroll:SetPoint("TOPLEFT", self.reset, "BOTTOMRIGHT", pixelScale(2), pixelScale(- 1 - (curPos - 1) / maxPos * total))
	self.scroll:Show()
end
...
local SetIcon = function(f, icon)
	if icon then
		f:SetWidth(pixelScale(s.width - s.lineheight - 4))
...
local onEnter = function(self)
	...
	GameTooltip:SetOwner(self, "ANCHOR_BOTTOMRIGHT", pixelScale(4), pixelScale(s.lineheight))
...
function window:GetLine(id)
	...
	f:SetStatusBarColor(0.6, 0.6, 0.6)
	f:SetWidth(s.width - 4)
	f:SetHeight(s.lineheight)

	if id == 0 then
		f:SetPoint("TOPRIGHT", self.reset, "BOTTOMRIGHT", pixelScale(-1), -pixelScale(1))
	else
		f:SetPoint("TOPRIGHT", lines[id-1], "BOTTOMRIGHT", 0, pixelScale(-s.linegap))
	end
	...
	value:SetPoint("RIGHT", pixelScale(-1), 0)
	...
	name:SetPoint("LEFT", icon, "RIGHT", pixelScale(1), 0)
	name:SetPoint("RIGHT", value, "LEFT", pixelScale(-1), 0)
